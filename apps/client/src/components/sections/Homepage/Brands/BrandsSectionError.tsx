...
import { revalidateBrandsCacheAction } from '@/lib/actions/brands.actions';

interface BrandsSectionErrorProps {
  error: Error;
  reset: () => void;
}
...
export const BrandsSectionError = ({ error, reset }: BrandsSectionErrorProps) => {
  ...
  const handleRetry = async () => {
    ...
    startTransition(async () => {
      ...
      const result = await revalidateBrandsCacheAction();
    ...
    <section className="bg-gunmetal px-4 py-16 lg:py-24">
      <div className="mx-auto max-w-[1360px]">
        ...
        <div className="flex min-h-[300px] flex-col items-center justify-center rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
          ...
          <h3 className="mb-2 text-xl font-semibold text-white">Nepodařilo se načíst značky</h3>
